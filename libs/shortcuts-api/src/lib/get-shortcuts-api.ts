...
export const getShortcutsApi: (shortcutsApiConfig: ShortcutsApiConfig) => HttpsFunction = (shortcutsApiConfig: ShortcutsApiConfig): HttpsFunction => runWith({
  ...
  .onRequest(async (request: Request, response: Response): Promise<void> => request.body["accessToken"] === shortcutsApiConfig.accessToken && request.secure ? (async (firestore: Firestore): Promise<void> => request.body["operation"] === "get all" ? (firestore.collection("environment").doc("private") as DocumentReference<PrivateEnvironmentDocument>).get().then<void>((privateEnvironmentDocumentSnapshot: DocumentSnapshot<PrivateEnvironmentDocument>): void => response.json(privateEnvironmentDocumentSnapshot.data()).end() && void(0)) : request.body["operation"] === "reset focus" ? (firestore.collection("environment").doc("private") as DocumentReference<PrivateEnvironmentDocument>).get().then<void>((privateEnvironmentDocumentSnapshot: DocumentSnapshot<PrivateEnvironmentDocument>): Promise<void> => (async (privateEnvironmentDocument: PrivateEnvironmentDocument | undefined): Promise<void> => privateEnvironmentDocument ? (firestore.collection("environment").doc("private") as DocumentReference<PrivateEnvironmentDocument>).update({
    ...
    "focusPrior": privateEnvironmentDocument.focus,
  }).end() && (firestore.collection("environment").doc("private") as DocumentReference<PrivateEnvironmentDocument>).update({
    "focus": request.body["focus"],
    "focusPrior": privateEnvironmentDocument.focus,
    ...
    "location": privateEnvironmentDocument.location === "away" ? shortcutsApiConfig.homeName : "away",
  }).end() && (firestore.collection("environment").doc("private") as DocumentReference<PrivateEnvironmentDocument>).update({
    "location": privateEnvironmentDocument.location === "away" ? shortcutsApiConfig.homeName : "away",
